...
//			elist[i]="\""+flist[i]+"\"";
	...
	run("Crop");
	Dialog.create("Remove outliers option");
	Dialog.addCheckbox("Two-step process after thresholding?", 0);
	Dialog.addMessage("Leave unchecked to run single step only (recommended)");
	Dialog.show();
	steps=Dialog.getCheckbox();
	Particles();
	runoptions();
	...
	function runoptions(){
		...
		if (runchoice==predef) {
			...
			Cncl="Cancel and return to run options";
			defaults=newArray(8);
			Array.fill(defaults, 0);
			labels=newArray(CaS,plagio,silica,FeTi,Cpx,Gls,Ctm1, Cncl);
			...
			Dialog.addCheckboxGroup(8, 1, labels, defaults);
			Dialog.show();
